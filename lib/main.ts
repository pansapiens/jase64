...
export default class Jase64 {
    ...
    // JSON->Base64
    public static base64json(data: any): string {
        return btoa(JSON.stringify(data));
    }

    // JSON->MsgPack->Base64
    public static base64MsgPackedjson(data: any): string {
    ...
    */
    ...
    public static compress(data: any, gz: boolean = true): string {
        let base64ed: string;
        const packed = MsgPack.encode(data);
        if (gz) {
            const gzed = gzip.zip(
                String.fromCharCode.apply(null, new Uint8Array(packed))
            );
    ...
    public static decompress(b64url: string): any {
